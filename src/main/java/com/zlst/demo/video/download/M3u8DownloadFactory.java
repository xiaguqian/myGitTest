package com.zlst.demo.video.download;
...
import com.zlst.demo.video.utils.StringUtils;
...
import static com.zlst.demo.video.utils.Constant.FILESEPARATOR;
...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        //链接连接超时时间（单位：毫秒）
        private long timeoutMillisecond = 3000L;
...
//            System.out.println("虎丘的usrl" + tsUrl);
         ...
         * 下载视频
         */
        private TaskExecutor fixedThreadPool;

        public void setFixedThreadPool(TaskExecutor fixedThreadPool) {
...
//                //轮询是否下载成功,如果下载未完成就会陷入此循环
                ...
                while (a) {
                    try {
                        ...
                        Thread.sleep(5000L);
                        if (consume%40 ==0 ) {
                            Log.i("已用时" + consume*5 + "秒！\t下载速度：" + StringUtils.convertToDownloadSpeed(new BigDecimal(downloadBytes.toString()).subtract(bigDecimal), 3) + "/s");
...
//            startListener(fixedThreadPool);
        ...
        private Thread getThread(String urls, int i) {
            return new Thread(() -> {
//                System.out.println("新的线程开始执行" + Thread.currentThread().getName());
                int count = 1;
                ...
                while (count <= retryCount) {
                    try {
                        ...
                        int len;
                        //将未解密的ts片段写入文件
//                        System.out.println("未解密未写入未读取开始报异常");
                        ...
                        outputStream.flush();
                        inputStream.close();
//                        System.out.println("未读取已经写入未解密报异常");
...
//                        System.out.println("所有步骤完成，未将文件添加至列表");
                        ...
                        if (count%10 ==0) {
                            Log.d("第" + count + "获取链接重试！\t" + urls+"，此时recount是" +retryCount);
                        }
                        if (count >=60) {
                            break;
                        }
...
//                Log.i(urls + "下载完毕！\t已完成" + finishedCount + "个，还剩" + (tsSet.size() - finishedCount) + "个！");
        ...
        private String getTsUrl() {
            ...
            for (String s : split) {
                ...
                if (s.contains(".m3u8")) {//这里判断，输入.m3u8即为m3u8地址但也可能是直接输入的第二个m3u8，
...
//            这儿返回值不为空，说明被加密了
        ...
        public StringBuilder getUrlContent(String urls, boolean isKey) {
            System.out.println("m3u8链接地址是" + urls + "是否用于获取key" + isKey);
            int count = 1;
            HttpURLConnection httpURLConnection = null;
...
//            Security.addProvider(new BouncyCastleProvider());
        ...
        private void checkField() {
            System.out.println("下载地址是"+DOWNLOADURL);
...
//            System.out.println("此时不为空，说明反复调用不删除");
            destroied();
            getInstance(downloadUrl);
        }
        return m3u8Download;
    }
